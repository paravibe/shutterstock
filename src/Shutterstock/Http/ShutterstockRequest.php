...
class ShutterstockRequest {
  ...
  public function __construct($requestType, $endpoint, $accessToken, $baseUrl, $apiVersion) {
    ...
    if (!$this->accessToken) {
      throw new ShutterstockException(ShutterstockConstants::ERROR_ACCESS_TOKEN);
    }
  ...
  public function execute($client = NULL) {
    ...
    try {
      ...
      $error = new ShutterstockResponse($result);

      throw new ShutterstockException($error->getBody(), $error->getHttpStatusCode());
    }

    return new ShutterstockResponse($result);
  }
   ...
   *
   * @return \Shutterstock\Main\Http\ShutterstockRequest
   *   ShutterstockRequest object.
   */
  ...
  private function getRequestUrl() {
    $url = $this->apiVersion . '/' . $this->endpoint;
